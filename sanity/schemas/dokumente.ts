...
export const seiteTyp = defineType({
  ...
  fields: [
    ...
    defineField({ name: "hero", title: "Grosser Seitenkopf (Startseite)", type: "object", group: "kopf", fields: [defineField({ name: "kurzzeile", title: "Kurzzeile", type: "string" }), defineField({ name: "titel", title: "Titel", type: "string", validation: (r) => r.required().max(90) }), defineField({ name: "text", title: "Text", type: "text", rows: 3, validation: (r) => r.max(220) }), defineField({ name: "knopf", title: "Knopf", type: "link" }), defineField({ name: "zweiterKnopf", title: "Zweiter Knopf", type: "link" }), defineField({ name: "fakten", title: "Vertrauensfakten (nur Startseite)", type: "array", of: [{ type: "string" }], description: "Kurze, belegte Aussagen wie «Krankenkassen anerkannt». Keine Bewertungen oder Zahlen ohne Quelle.", validation: (r) => r.max(4) }), defineField({ name: "angebot", title: "Angebotskarte (nur Startseite)", type: "object", description: "Ein belegtes Angebot aus der Preisliste mit Link. Kein Streichpreis, keine Verknappung.", fields: [defineField({ name: "etikett", title: "Etikett", type: "string" }), defineField({ name: "titel", title: "Titel", type: "string", validation: (r) => r.required() }), defineField({ name: "preisText", title: "Preis (Anzeige)", type: "string", description: "z. B. «CHF 399.–» oder «ab CHF 399.–»", validation: (r) => r.required() }), defineField({ name: "hinweis", title: "Hinweis", type: "string" }), defineField({ name: "link", title: "Link", type: "link", validation: (r) => r.required() })] }), defineField({ name: "bild", title: "Bild", type: "bild" }), defineField({ name: "variante", title: "Variante", type: "string", options: { list: ["start", "seite", "artikel"], layout: "radio" }, initialValue: "seite", validation: (r) => r.required() })] }),
