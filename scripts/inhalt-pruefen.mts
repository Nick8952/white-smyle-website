...
for (const f of fragebogen) { const ort = `Fragebogen ${f.id}`; if (!f.titel || !f.slug || !f.quelle) fehler.push(`${ort}: Pflichtfeld fehlt`); if (f.originalUrl && !/^https:\/\//.test(f.originalUrl as string)) fehler.push(`${ort}: originalUrl muss https sein`); const fk = new Set<string>(); for (const q of (f.fragen as Roh[]) ?? []) { if (!q._key || fk.has(q._key as string)) fehler.push(`${ort}: Frage-_key fehlt/doppelt`); fk.add(q._key as string); if (!["jaNein", "auswahl", "text"].includes(q.art as string)) fehler.push(`${ort}: Frageart ungültig`); if (q.art === "auswahl" && !(q.optionen as unknown[])?.length) fehler.push(`${ort}: Auswahl ohne Optionen`); } }
